...
const getFollowingPeopleIdsOfUser = async username => {
  ...
  INNERJOIN user ON user.user_id = follower.follower_user_id
...
const tweetAccessVerfication = async (request, response, next) => {
  ...
  * 
  FROM tweet INNERJOIN follower
  ON tweet.user_id = follower.following_user_id 
  WHERE tweet.tweet_id = '${tweet_id}'AND follower_user_id = '${userId}';`
  const tweet = awaitdb.get(getTweetQuery)
  if (tweet === undefined) {
    response.status(401)
  ...
  FROM userINNERJOIN tweet ON user.user_id = tweet.user_id
  WHERE
  user.user_id IN(${followingPeopleIds})
  ORDER BY date_time DESC
  LIMIT  4 ;
  `
    const tweets = await db.all(getTweeetsQuery)
  ...
  app.get('/user/followers/', authentication, async (request, response) => {
    ...
    WHERE
      follower_user_id = '${userId};`
    const followers = await db.get(getFollowingUsersQuery)
  ...
  app.get(
    ...
    async (request, response) => {
      ...
      const userlikes = await db.get(getLikesQuery)
      const userArray = userlikes.map(eachUser => eachUser.username)
  ...
  app.get(
    ...
    async (request, response) => {
      ...
      WHERE tweet_id = '${tweetId}';`
      const repliedusers = await db.all(getTweetQuery)
